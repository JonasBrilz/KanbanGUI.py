...
from . import boardUtil
from .itemGUI import icon
...
def startup() -> None:
    ...
    boardUtil.websiteButton(
        refFrame, 0, 1, "GitHub", "https://github.com/JonasBrilz/KanbanGUI.py"
    )
    boardUtil.fileButton(
        refFrame, 0, 2, "Documentation", "resources/documentation_german.pdf"
    )
    ...
    tb.Label(refFrame, style="KanbanGUI.TLabel", text="Draft").grid(
        row=2, column=0
    )  # draftLabel
    tb.Label(refFrame, style="KanbanGUI.TLabel", text="Open").grid(
        row=2, column=1
    )  # openLabel
    tb.Label(refFrame, style="KanbanGUI.TLabel", text="Active").grid(
        row=2, column=2
    )  # activeLabel
    tb.Label(refFrame, style="KanbanGUI.TLabel", text="Complete").grid(
        row=2, column=3
    )  # completeLabel
    tb.Label(refFrame, style="KanbanGUI.TLabel", text="Discarded").grid(
        row=2, column=4
    )  # discardedLabel
